...
function displayOrders() {
    ...
    db.collection("orders").where("done", "==", false).onSnapshot((ordersSnapshot) => {
      ...
      ordersSnapshot.forEach((docSnapshot) => {
        ...
        orderDiv.innerHTML = `
          ...
          <button class="btn btn-complete" onclick="markOrderAsDone('${orderId}')">✔️ Complete Order</button>
        `;
  ...
  async function markOrderAsDone(orderId) {
    try {
      ...
      await orderDocRef.update({ done: true });
